...
def best_first_graph_search(problem, f):
    ...
    # print("inside best_first_graph_search ---------->>>>>")
    f = memoize(f, 'f')
    node = Node(problem.initial)
    if problem.goal_test(node.state):
        return node
    frontier = PriorityQueue(min, f)
    # print("inside best_first_graph_search - ---------> >>>> before frontier.append(node)")
    frontier.append(node)
    # print("inside best_first_graph_search - ---------> >>>> after frontier.append(node)")
    explored = set()
    while frontier:
        #print("inside best_first_graph_search - ---------> >> >>inside while frontier: before frontier.pop()")
        node = frontier.pop()
        if problem.goal_test(node.state):
            #print("inside best_first_graph_search - ---------> >>>>just before return")
            return node
        explored.add(node.state)
        for child in node.expand(problem):
            # print("inside best_first_graph_search - ---------> >> >>inside while frontier: inside for child loop")
            if child.state not in explored and child not in frontier:
                # print("inside best_first_graph_search - ---------> >> >>inside while frontier: before frontier.append()")
                frontier.append(child)
                # print("inside best_first_graph_search - ---------> >> >>inside while frontier: after frontier.append()")
