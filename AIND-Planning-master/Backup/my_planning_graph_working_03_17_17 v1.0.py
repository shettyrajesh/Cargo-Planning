...
class PlanningGraph():
    ...
    def create_graph(self):
        ...
        while not leveled:
            print("Inside create GRAPH ---> while not level ----> level - ")
            print(level)
            self.add_action_level(level)
            # print("self.a_levels[level] ")
            # print(self.a_levels[level])
    ...
    def add_literal_level(self, level):
        ...
        print("Inside add_literal_level ----->")
        print("len(parent_nodes)")
        print(len(parent_nodes))
        # print("len(literal_node)")
        # print(len(literal_node))
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #return False
        return not set(node_a1.action.effect_add).isdisjoint(node_a2.action.effect_rem) or \
               not set(node_a2.action.effect_add).isdisjoint(node_a1.action.effect_rem)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        precond_pos_a1 = set(node_a1.action.precond_pos)
        precond_pos_a2 = set(node_a2.action.precond_pos)
        action_preconds_mutex = not (precond_pos_a1.issubset(precond_pos_a2) or precond_pos_a2.issubset(precond_pos_a1))
        parents_mutex = False
        for parent in node_a1.parents:
            if len(node_a2.parents.intersection(parent.mutex)) > 0:
                parents_mutex = True
            if parents_mutex:
                break
        for parent in node_a2.parents:
            if len(node_a1.parents.intersection(parent.mutex)) > 0:
                parents_mutex = True
            if parents_mutex:
                break
        return action_preconds_mutex or parents_mutex
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            for s_level in self.s_levels:
                pos_literals = [s.literal for s in s_level if s.is_pos]
                if goal in pos_literals:
